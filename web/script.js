    ...
    onTabChange(tabId) {
        switch(tabId) {
            ...
            case '#config':
                ...
                refreshCacheStats();
                // Load Google Photos data when config tab is active
                loadGooglePhotosData();
                break;
            case '#logs':
                this.loadLogs();
                this.startLogAutoRefresh();
...
async function authorizeGooglePhotos() {
    try {
        showGooglePhotosAlert('Getting authorization URL...', 'info');
        const response = await fetch('/api/google-photos/authorize');
        const data = await response.json();
        if (data.success) {
            // Open authorization URL in new tab
            window.open(data.data.authorization_url, '_blank');
            showGooglePhotosAlert('Authorization URL opened in new tab. After authorization, copy the code from the URL and paste it in the input field below.', 'success');
        } else {
            showGooglePhotosAlert('Error getting authorization URL: ' + data.error, 'danger');
        }
    } catch (error) {
        showGooglePhotosAlert('Failed to get authorization URL: ' + error.message, 'danger');
    }
}

async function exchangeGooglePhotosCode() {
    const codeInput = document.getElementById('google-photos-auth-code');
    const code = codeInput.value.trim();
    
    if (!code) {
        showGooglePhotosAlert('Please enter the authorization code', 'warning');
        return;
    }
    
    try {
        showGooglePhotosAlert('Exchanging authorization code for tokens...', 'info');
        ...
        const data = await response.json();
        if (data.success) {
            showGooglePhotosAlert('Authorization successful! Google Photos API is now configured.', 'success');
            // Clear the input field
            codeInput.value = '';
            // Refresh the authentication status
            await testGooglePhotosAuth();
        } else {
            showGooglePhotosAlert('Error exchanging authorization code: ' + data.error, 'danger');
        }
    } catch (error) {
        showGooglePhotosAlert('Failed to exchange authorization code: ' + error.message, 'danger');
    }
}

async function checkGooglePhotosSync() {
    try {
        const response = await fetch('/api/google-photos/sync-status');
        const data = await response.json();
        
        if (data.success) {
            updateGooglePhotosSyncStatus(data.data);
            showGooglePhotosAlert('Sync status checked successfully', 'success');
        } else {
            showGooglePhotosAlert('Error checking Google Photos sync: ' + data.error, 'danger');
        }
    } catch (error) {
        showGooglePhotosAlert('Failed to check Google Photos sync: ' + error.message, 'danger');
    }
}
...
async function testGooglePhotosAuth() {
    try {
        ...
        if (data.success) {
            updateGooglePhotosAuthStatus(data.data);
            showGooglePhotosAlert('Authentication test completed', 'success');
        } else {
            showGooglePhotosAlert('Error testing Google Photos auth: ' + data.error, 'danger');
        }
    } catch (error) {
        showGooglePhotosAlert('Failed to test Google Photos auth: ' + error.message, 'danger');
    }
}

async function viewGooglePhotosLogs() {
    try {
        const response = await fetch('/api/google-photos/logs');
        const data = await response.json();
        
        if (data.success) {
            showGooglePhotosLogs(data.data);
        } else {
            showGooglePhotosAlert('Error loading Google Photos logs: ' + data.error, 'danger');
        }
    } catch (error) {
        showGooglePhotosAlert('Failed to load Google Photos logs: ' + error.message, 'danger');
    }
}

function updateGooglePhotosAuthStatus(authData) {
    const container = document.getElementById('google-photos-auth-status');
    const statusBadge = document.getElementById('google-photos-status-badge');
    
    if (authData.authenticated) {
        container.innerHTML = `
            <div class="text-center">
                <div class="mb-3">
                    <i class="fas fa-shield-check text-success" style="font-size: 2.5rem;"></i>
                </div>
                <div class="fw-bold text-success fs-5">Authenticated</div>
                <small class="text-muted">Token expires: ${authData.tokenExpiry || 'Unknown'}</small>
                <div class="mt-2">
        ...
        container.innerHTML = `
            <div class="text-center">
                ...
                <div class="fw-bold text-warning fs-5">Not Authenticated</div>
                <small class="text-muted">${authData.error || 'Authentication required'}</small>
                <div class="mt-2">
...
function updateGooglePhotosSyncStatus(syncData) {
    const container = document.getElementById('google-photos-sync-status');
    
    if (syncData.enabled) {
        const statusClass = syncData.syncRate >= 80 ? 'success' : syncData.syncRate >= 50 ? 'warning' : 'danger';
        const statusIcon = syncData.syncRate >= 80 ? 'check-circle' : syncData.syncRate >= 50 ? 'exclamation-triangle' : 'times-circle';
        const progressClass = syncData.syncRate >= 80 ? 'bg-success' : syncData.syncRate >= 50 ? 'bg-warning' : 'bg-danger';
        
        container.innerHTML = `
            <div class="text-center">
                <div class="mb-3">
                    <i class="fas fa-${statusIcon} text-${statusClass}" style="font-size: 2.5rem;"></i>
                </div>
                <div class="fw-bold text-${statusClass} fs-4">${syncData.syncRate.toFixed(1)}% Synced</div>
                <small class="text-muted">${syncData.syncedFiles}/${syncData.totalFiles} files</small>
                <div class="mt-3">
                    <div class="progress google-photos-progress">
                        <div class="progress-bar ${progressClass}" role="progressbar" style="width: ${syncData.syncRate}%" 
                             aria-valuenow="${syncData.syncRate}" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                </div>
                <div class="mt-2">
                    <span class="badge bg-${statusClass}">${syncData.syncRate >= 80 ? 'Excellent' : syncData.syncRate >= 50 ? 'Good' : 'Needs Attention'}</span>
                </div>
            </div>
        `;
        
        // Update stats
        updateGooglePhotosStats(syncData);
    } else {
        container.innerHTML = `
            <div class="text-center">
                <div class="mb-3">
                ...
                <div class="fw-bold text-info fs-5">Sync Check Disabled</div>
                <small class="text-muted">Enable in configuration</small>
                <div class="mt-2">
...
function updateGooglePhotosStats(syncData) {
    const container = document.getElementById('google-photos-stats');
    
    container.innerHTML = `
        <div class="row g-3">
            <div class="col-md-3">
                <div class="card border-0 bg-primary text-white text-center h-100 google-photos-stats-card">
                    <div class="card-body">
                        <i class="fas fa-folder-open fa-2x mb-2 google-photos-status-icon"></i>
                        <div class="h3 mb-1">${syncData.totalFiles || 0}</div>
                        <small>Total Files</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-0 bg-success text-white text-center h-100 google-photos-stats-card">
                    <div class="card-body">
                        <i class="fas fa-check-circle fa-2x mb-2 google-photos-status-icon"></i>
                        <div class="h3 mb-1">${syncData.syncedFiles || 0}</div>
                        <small>Synced</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-0 bg-warning text-white text-center h-100 google-photos-stats-card">
                    <div class="card-body">
                        <i class="fas fa-exclamation-triangle fa-2x mb-2 google-photos-status-icon"></i>
                        <div class="h3 mb-1">${syncData.notSyncedFiles || 0}</div>
                        <small>Not Synced</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-0 bg-info text-white text-center h-100 google-photos-stats-card">
                    <div class="card-body">
                        <i class="fas fa-percentage fa-2x mb-2 google-photos-status-icon"></i>
                        <div class="h3 mb-1">${syncData.syncRate.toFixed(1)}%</div>
                        <small>Sync Rate</small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="alert alert-light border-0 text-center">
                    ...
                    <small class="text-muted">Last checked: ${new Date(syncData.lastCheck).toLocaleString()}</small>
                </div>
            </div>
        </div>
        ` : ''}
...
function showGooglePhotosLogs(logs) {
    // Create a modal to show logs
    const modalHtml = `
        <div class="modal fade" id="googlePhotosLogsModal" tabindex="-1">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info">
                            ...
                            <strong>Log Information:</strong> This shows recent Google Photos sync verification logs and API activity.
                        </div>
                        <pre class="bg-dark text-light p-3 rounded" style="max-height: 500px; overflow-y: auto; font-family: 'Courier New', monospace;">${logs}</pre>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="refreshGooglePhotosData()">
    ...
    const existingModal = document.getElementById('googlePhotosLogsModal');
    if (existingModal) {
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('googlePhotosLogsModal'));
    modal.show();
}

// Load Google Photos data on dashboard load
async function loadGooglePhotosData() {
    await Promise.all([
        testGooglePhotosAuth(),
        checkGooglePhotosSync()
    ]);
}

// Refresh all Google Photos data
async function refreshGooglePhotosData() {
    ...
    if (statusBadge) {
        ...
        setTimeout(() => {
            ...
            statusBadge.className = 'badge bg-primary text-white ms-2';
        }, 2000);
    }
}

function showGooglePhotosAlert(message, type = 'info') {
    // Create a simple alert function for Google Photos
    const alertContainer = document.getElementById('google-photos-auth-status');
