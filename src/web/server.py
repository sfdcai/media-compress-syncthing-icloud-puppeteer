        ...
        try:
            ...
            try:
                ...
                if url and key:
                    ...
                    # Get total files
                    result = supabase.table('media_files').select('id', count='exact').execute()
                    stats['total_files'] = result.count or 0
                    
                    # Get files processed today
                    today = datetime.now().date()
                    result = supabase.table('media_files').select('id', count='exact').gte('created_at', str(today)).execute()
                    stats['files_today'] = result.count or 0
                    
                    # Get success rate from pipeline logs
                    result = supabase.table('pipeline_logs').select('status').execute()
                    if result.data:
                        total_logs = len(result.data)
                        successful_logs = len([log for log in result.data if log.get('status') == 'success'])
                        stats['success_rate'] = (successful_logs / total_logs) * 100 if total_logs > 0 else 0
    ...
    def get_configuration(self):
        ...
        try:
            ...
            config_descriptions = {
                ...
                'ENABLE_PIXEL_UPLOAD': 'Enable upload to Google Pixel (Google Photos). When enabled, files will be synced to Google Photos via Syncthing.',
...
def icloudpd_2fa():
    ...
    try:
        ...
        # Update 2FA request in Supabase
        try:
            from supabase import create_client, Client
            ...
            if url and key:
                ...
                result = supabase.table('twofa_requests').update({
                    'code': code,
                    'status': 'completed',
                    'completed_at': 'now()'
...
def icloudpd_status():
    ...
    try:
        ...
        # Check 2FA request status in Supabase
        try:
            from supabase import create_client, Client
            ...
            if url and key:
                ...
                result = supabase.table('twofa_requests').select('*').eq('request_id', request_id).execute()
...
def send_telegram_status():
    ...
    try:
        ...
        sys.path.insert(0, str(project_root))
        from scripts.enhanced_telegram_bot import EnhancedTelegramBot
...
def get_cache_stats():
    ...
    try:
        from scripts.local_db_manager import get_db_manager
...
def get_database_tables():
    ...
    try:
        ...
        cursor.execute("""
            SELECT table_name, table_owner, 'table' as table_type
