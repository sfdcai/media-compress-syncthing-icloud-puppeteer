...
def process_media_file(file_path, output_dir, is_initial=True):
    ...
    try:
        ...
        # Check if file is already compressed and up to date
        if os.path.exists(output_path):
            # Check if compressed file is newer than original
            original_mtime = os.path.getmtime(file_path)
            compressed_mtime = os.path.getmtime(output_path)
            
            if compressed_mtime > original_mtime:
                log_step("compression", f"File {filename} already compressed and up to date", "info")
                return True  # Return True since file is already compressed
        ...
        # Check if compression is enabled for this file type
        if is_image_file(file_path):
            if not get_feature_toggle("ENABLE_IMAGE_COMPRESSION"):
                log_step("compression", f"Image compression disabled, skipping {filename}", "info")
                return False
            success, compression_ratio = compress_image_progressive(file_path, output_path, is_initial)
        elif is_video_file(file_path):
            if not get_feature_toggle("ENABLE_VIDEO_COMPRESSION"):
                log_step("compression", f"Video compression disabled, skipping {filename}", "info")
                return False
        ...
        if success:
            ...
            # Update database with compression ratio
            update_compression_ratio_in_database(file_path, compression_ratio)
...
def update_compression_ratio_in_database(file_path, compression_ratio):
    """Update compression ratio in database for the original file"""
    try:
        from core.local_db_manager import get_db_manager
        db = get_db_manager()
        
        # Find the media file record by file path
        query = "SELECT id FROM media_files WHERE file_path = %s"
        result = db._execute_query(query, (file_path,), fetch=True)
        
        if result and len(result) > 0:
            file_id = result[0]['id']
            
            # Update compression ratio
            update_query = "UPDATE media_files SET compression_ratio = %s WHERE id = %s"
            db._execute_query(update_query, (compression_ratio, file_id))
            
            log_step("compression", f"Updated compression ratio {compression_ratio:.1f}% for {os.path.basename(file_path)}", "info")
        else:
            log_step("compression", f"No database record found for {file_path}", "warning")
            
    except Exception as e:
        log_step("compression", f"Failed to update compression ratio in database: {e}", "error")
