...
from utils.utils import (
    ...
    calculate_file_hash, copy_file_with_sudo
)
    ...
    try:
        ...
        for file_path in files:
            try:
                ...
                # Copy file
                # Use sudo copy for NAS mounts
                if not copy_file_with_sudo(file_path, dest_path):
                    log_step("batch_preparation", f"Failed to copy {file_path} to {dest_path}", "error")
                    continue
        ...
        batch_id = create_batch_record(
            ...
            total_size=int(total_size_gb * 1024 * 1024 * 1024)  # Convert GB to bytes
        )
