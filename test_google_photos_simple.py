...
pytestmark = pytest.mark.skipif(
    not RUN_PROD_TESTS,
    reason="Set RUN_PROD_TESTS=1 to enable Google Photos API smoke tests."
)
...
def test_google_photos():
    ...
    if not os.path.exists(token_file):
        ...
        return _test_result(False, "Google Photos token file not found")
    ...
    if not access_token:
        ...
        return _test_result(False, "Google Photos access token missing")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return _test_result(False, f"Google Photos mediaItems request failed with {response.status_code}")

    except Exception as e:
        print(f"❌ API call error: {e}")
        return _test_result(False, f"Google Photos mediaItems API error: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return _test_result(False, f"Google Photos search request failed with {response.status_code}")

    except Exception as e:
        print(f"❌ Search API error: {e}")
        return _test_result(False, f"Google Photos search API error: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return _test_result(False, f"Google Photos albums request failed with {response.status_code}")

    except Exception as e:
        print(f"❌ Albums API error: {e}")
        return _test_result(False, f"Google Photos albums API error: {e}")
